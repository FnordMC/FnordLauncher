...
Name "Fnord Launcher"
InstallDir "$LOCALAPPDATA\Programs\FnordLauncher"
InstallDirRegKey HKCU "Software\FnordLauncher" "InstallDir"
RequestExecutionLevel user
OutFile "../FnordLauncher-Setup.exe"

!define MUI_ICON "../program_info/fnordlauncher.ico"

!define UNINST_KEY "Software\Microsoft\Windows\CurrentVersion\Uninstall\FnordLauncher"
...
!define MUI_FINISHPAGE_RUN "$InstDir\fnordlauncher.exe"
...
Section "Fnord Launcher"

  SectionIn RO

  nsExec::Exec /TIMEOUT=2000 'TaskKill /IM fnordlauncher.exe /F'

  SetOutPath $INSTDIR

  File "fnordlauncher.exe"
  File "fnordlauncher_filelink.exe"
  File "fnordlauncher_updater.exe"
  File "qt.conf"
  File "qtlogging.ini"
  File *.dll
  File /r "iconengines"
  ...
  WriteRegStr HKCU Software\FnordLauncher "InstallDir" "$INSTDIR"

  ; Write the URL Handler into registry for curseforge
  WriteRegStr HKCU Software\Classes\curseforge "URL Protocol" ""
  WriteRegStr HKCU Software\Classes\curseforge\shell\open\command "" '"$INSTDIR\fnordlauncher.exe" "%1"'

; Write the URL Handler into registry for prismlauncher
  WriteRegStr HKCU Software\Classes\prismlauncher "URL Protocol" ""
  WriteRegStr HKCU Software\Classes\prismlauncher\shell\open\command "" '"$INSTDIR\fnordlauncher.exe" "%1"'

  ; Write the uninstall keys for Windows
  ${GetParameters} $R0
  ${GetOptions} $R0 "/NoUninstaller" $R1
  ${If} ${Errors}
    WriteRegStr HKCU "${UNINST_KEY}" "DisplayName" "Fnord Launcher"
    WriteRegStr HKCU "${UNINST_KEY}" "DisplayIcon" "$INSTDIR\fnordlauncher.exe"
    ...
    WriteRegStr HKCU "${UNINST_KEY}" "Publisher" "Fnord Launcher Contributors"
    WriteRegStr HKCU "${UNINST_KEY}" "Version" "9.4.2.0"
    WriteRegStr HKCU "${UNINST_KEY}" "DisplayVersion" "9.4.2"
    WriteRegStr HKCU "${UNINST_KEY}" "VersionMajor" "9"
    WriteRegStr HKCU "${UNINST_KEY}" "VersionMinor" "4"
...
Section "Start Menu Shortcut" SM_SHORTCUTS
  ...
  CreateShortcut "$SMPROGRAMS\Fnord Launcher.lnk" "$INSTDIR\fnordlauncher.exe" "" "$INSTDIR\fnordlauncher.exe" 0

SectionEnd

Section /o "Desktop Shortcut" DESKTOP_SHORTCUTS

  CreateShortcut "$DESKTOP\Fnord Launcher.lnk" "$INSTDIR\fnordlauncher.exe" "" "$INSTDIR\fnordlauncher.exe" 0

SectionEnd


!define APPID "FnordLauncher.App"
!define APPEXE "fnordlauncher.exe"
!define APPICON "$INSTDIR\${APPEXE},0"
!define APPDESCRIPTION "Fnord Launcher"
!define APPNAME "Fnord Launcher"
!define APPCMDTEXT "Fnord Launcher"
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Microsoft\Windows\CurrentVersion\Uninstall\FnordLauncher"
  DeleteRegKey HKCU SOFTWARE\FnordLauncher

  Delete $INSTDIR\fnordlauncher.exe
  Delete $INSTDIR\fnordlauncher_filelink.exe
  Delete $INSTDIR\fnordlauncher_updater.exe
  ...
  Delete "$SMPROGRAMS\Fnord Launcher.lnk"
  Delete "$DESKTOP\Fnord Launcher.lnk"
